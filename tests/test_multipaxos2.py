...
class TestMultiPaxos2:
	...
	def test_accept_fresh_value(self):
		...
		assert mp[3].accept_val == ("POST","user", "title", "test_accept_fresh_value")
	...
	def test_accept_failed(self):
		...
		mp[0].addToQueue(("POST","user", "title", "test_accept_failed"))
		assert mp[0].queue == [("POST","user", "title", "test_accept_failed")]
	...
	def test_accept_twice(self):
		...
		time.sleep(.1)
		mp[0].prepare()
		mp[0].addToQueue(("POST","user", "title", "test_accept_twice"))
		mp[0].accept()
		time.sleep(.1)
		for i in range(3):
			assert mp[i].accept_val == ("POST","user", "title", "test_accept_twice")
		...
		mp[4].addToQueue(("POST","wrong user", "title", "test_accept_twice"))
		mp[4].accept()
		for i in range(5):
			assert mp[i].accept_val == ("POST","user", "title", "test_accept_twice")
		for i in range(1,5):
	...
	def test_accept_twice_fail(self):
		...
		time.sleep(.4)
		assert mp[1].accept_val == None
		assert mp[2].accept_val == None
		assert mp[3].accept_val == None
		assert mp[4].accept_val == None
		time.sleep(.1)
		assert mp[0].queue == []
		mp[1].net.fix_link(0)
		time.sleep(.1)
		mp[1].accept(("POST","this is user 1", "title", "test_accept_twice_fail"))
		time.sleep(.1)
		for i in range(5):
			assert mp[i].accept_val == ("POST","this is user 1", "title", "test_accept_twice_fail")
		assert mp[i].queue == []

	def test_forward_queue(self):
		mp = [MultiPaxos(Network(i), i, Blog(), disable_queue=True) for i in range(5)]
		q = [("POST","user", "title", f"test_forward_queue {i}") for i in range(5)]
		for m in q:
			mp[0].addToQueue(m)
		assert mp[0].queue == q
		mp[2].prepare()
